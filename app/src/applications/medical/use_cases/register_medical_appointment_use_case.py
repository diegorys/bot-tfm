...
from src.applications.medical.domain.medical_appointment_repository import MedicalAppointmentRepository
...
class RegisterMedicalAppointmentUseCase:
    ...
    def execute(self, medicalAppointment: MedicalAppointment):
        print(f"SAVE MEDICAL APPOINTMENT {medicalAppointment.speciality.name} AT {medicalAppointment.date.date}")
        event: Event = Event(
            "RECORDAR_CITA_MEDICA", medicalAppointment.user, medicalAppointment.speciality.name, medicalAppointment.date.date
        )
